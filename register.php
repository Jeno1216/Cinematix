...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($pass != $cpass) {
            ...
            $insert = "INSERT INTO cinematix_user (name, email, password, cpassword, profile_picture) 
         VALUES('$name', '$email', '$pass', '$cpass', '$images')";
            $query_run = mysqli_query($conn, $insert);
            header('location:signin.php'); // point to login_form.php

            if ($query_run) {
                // this is to insert the image to the folder profile_pictures
                move_uploaded_file($_FILES["images"]["tmp_name"], "profile_pictures/" . $_FILES["images"]["name"]);
            }
        }
    }
}; ?>
